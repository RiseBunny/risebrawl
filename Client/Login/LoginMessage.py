...
class LoginMessage(BSMessageReader):
    ...
    def process(self):
        ...
        if self.player.low_id in settings['banID']:
            ...
            LoginFailedMessage(self.client, self.player, "Упс, кажись вы были заблокированы навсегда! Ошибка? Подать на аппеляцию можно написав админу - @lekma").send()
        ...
        if self.player.low_id >= 2:
            ...
            LoginFailedMessage(self.client, self.player, "Hesap bulunamadı tüm oyun verilerini silin!").send()